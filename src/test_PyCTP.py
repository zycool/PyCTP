...
class PyCTP_Market_API(PyCTP.CThostFtdcMdApi):
    ...
    def UnSubMarketData(self, InstrumentID):
        """ 退订行情 """
        self.__rsp_UnSubMarketData = dict(results=[], ErrorID=0, event=threading.Event(),
                                          RequestID=self.__IncRequestID())
...
class PyCTP_Market(PyCTP_Market_API):
    ...
    def OnRtnDepthMarketData(self, DepthMarketData):
        ...
        tick = dict(InstrumentID=str(DepthMarketData['InstrumentID'], 'gb2312')
                ...
                str(DepthMarketData['ActionDay'] + DepthMarketData['UpdateTime'], 'gb2312'), '%Y%m%d%H:%M:%S').replace(
                microsecond=DepthMarketData['UpdateMillisec'] * 1000)
...
def __main__():
    ...
    time.sleep(1.0)
    print('查询交易所----->')
    for item in trader.QryExchange():
        print(item)
    time.sleep(1.0)
